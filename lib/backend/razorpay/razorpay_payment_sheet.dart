...
const _kProdRazorpayKeyId = 'rzp_live_SYv2FFeu4uSUBT';
const _kTestRazorpayKeyId = 'rzp_test_ST1TJFmdin3t51';
const _kBusinessName = 'Satrango';
...
class _RazorpayPaymentSheetState extends State<RazorpayPaymentSheet> {
  ...
  Future<void> _checkoutOrder() async {
    ...
    try {
      ...
      if (orderDetails.id == null || orderDetails.id!.isEmpty) {
        throw StateError('createOrder returned no order id');
      }
      if (mounted) {
    ...
    final orderId = _processingOrderDetails?.id;
    if (orderId != null && orderId.isNotEmpty) {
      final options = RazorpayOptions(
        key: razorpayKeyId,
        amount: _processingOrderDetails?.amount ?? widget.amount,
        currency: widget.currency,
      ...
      setState(() {
        _paymentStatus = PaymentStatus.failed;
        _paymentResponseMessage = 'Could not start payment. Please sign in and try again.';
      });
      widget.onPaymentStateChange(null);
      Future.delayed(
        const Duration(seconds: 2),
          ...
          children: [
            if (_paymentStatus == PaymentStatus.processing)
              LinearProgressIndicator(
                ...
                color: backgroundColor.applyAlpha(0.5),
                minHeight: 5,
              ),
            Padding(
